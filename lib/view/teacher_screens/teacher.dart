...
class _CourseFormScreenState extends State<CourseFormScreen> {
  ...
  Future<List<String>> _listImages(String folderPath) async {
    try {
      final files = await Supabase.instance.client.storage.from('uploads').list(path: folderPath);

      // تحويل كل ملف إلى رابط عام
      return files.map((f) => Supabase.instance.client.storage
          .from('uploads')
          .getPublicUrl('$folderPath/${f.name}')) // لا تضف .publicUrl
          .toList();
    } catch (e) {
  ...
  Future<void> _save() async {
    ...
    if (teacherId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('خطأ: لم يتم العثور على teacher_id')));
      return;
    }
    ...
    try {
      ...
      if (widget.course == null) {
        ok = await coursesService.createCourse(course);
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(isEdit ? 'Edit Course' : 'Create Course')),
      body: Padding(
            ...
            Form(
              ...
              child: Column(
                children: [
                  ...
                  TextFormField(controller: _thumb, decoration: const InputDecoration(labelText: 'Thumbnail URL'), readOnly: true),
                        ...
                        builder: (context) => FutureBuilder<List<String>>(
                          ...
                          builder: (context, snap) {
                            if (snap.connectionState == ConnectionState.waiting) {
                              ...
                              return AlertDialog(title: const Text('Error'), content: Text('Failed to load images: ${snap.error}'));
                            } else {
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                            ...
                                            itemBuilder: (ctx, i) {
                                              ...
                                              return GestureDetector(
                                                ...
                                                onLongPress: () async {
                                                  ...
                                                  if (ok == true) {
                                                    try {
                                                      await Supabase.instance.client.storage.from('uploads').remove(['$folderPath/$fileName']);
                                                      setState(() => images.removeAt(i));
                                                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم حذف الصورة ✅')));
                                                    } catch (e) {
                                                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('فشل حذف الصورة: $e')));
                                                    }
                                                  }
                                                },
                                                child: Padding(
                                      ...
                                      ElevatedButton(
                                        onPressed: () async {
                                          ...
                                          if (result != null) {
                                            ...
                                            try {
                                              ...
                                              // الحصول على رابط عام (String مباشرة)
                                        ...
                                        child: const Text('رفع صورة جديدة من الجهاز'),
                                      ),
                                    ],
                                  ),
                                ),
                  ...
                  const SizedBox(height: 16),
                  ElevatedButton(onPressed: _save, child: Text(isEdit ? 'Update' : 'Create')),
                ],
              ),
            ),
          ],
